...
export class FinanceNewsAgent extends AutonomousAgentAdapter {
    static readonly NAME = 'FinanceNewsAgent';
    ...
    static readonly USER_PROMPT = new UserPromptAdapter(
        PROMPTS.MISSIONS.ANIMATE_CHATROOM(
            'Important news, discussions or updates related to finance and markets',
        ),
    ...
    constructor(
        ...
        const tools = [
            availableTools.fetchChatBotMessages.finance,
            availableTools.getCurrentDate,
            availableTools.fetchPostsForFinance,
        ];

        super(FinanceNewsAgent.NAME, {
            logger,
            model,
            systemPrompt: FinanceNewsAgent.SYSTEM_PROMPT,
            tools,
        });
    }

    async run(): Promise<null | string> {
        const result = await super.run(FinanceNewsAgent.USER_PROMPT);
